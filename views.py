...
def sign_up(request: HttpRequest) -> HttpResponse:
    if request.method == 'POST':
        ...
        # 检查年龄
        try:
            birth_year = int(data_form.get('date_of_birth', '').split("-")[0])
            if (datetime.now().year - birth_year) < 12:
                form.add_error(None, 'You must be at least 12 years old to sign up')
                return render(request, 'registration/signup.html', {'form': form})
        except (ValueError, IndexError):
            form.add_error('date_of_birth', 'Invalid date format')
            return render(request, 'registration/signup.html', {'form': form})
        ...
        if form.is_valid():
            try:
                ...
                user.username = email  # 再次确保username被设置
                user.save()
